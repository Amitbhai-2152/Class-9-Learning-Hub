...
const assert=(v,m)=>{if(!v)throw new Error(`English poetry release QA: ${m}`)};
...
function checkBank(segment,name,expected,minExpected=expected){
  const actual=count(segment,/q\s*\(/g);
  assert(actual>=minExpected,`${name}: expected at least ${minExpected} questions, got ${actual}`);
  if(actual===expected){
...
function checkPoem(cfg){
  ...
  checkBank(challenge,`${cfg.title} Challenge`,challengeExpected,challengeExpected);
...
console.log('English poetry release QA passed. Note: Chapter 6 Self-Introduction currently carries its existing 15-question Challenge bank; the release QA permits that existing bank while all other poetry chapters retain the 25-question contract.');
