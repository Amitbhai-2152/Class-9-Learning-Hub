...
for(const [name,p,title,author,exact23] of specs){
 const t=read(p);bank(t,name,exact23);
 ...
 assert(t.includes('sections:['),`${name}: guided sections missing`);
 assert(count(t,"{title:'Part ")>=8,`${name}: insufficient guided reading`);
 assert(t.includes('wordStudy:'),`${name}: wordStudy missing`);
...
assert(/practice:[\s\S]*?45/.test(engine),'practice timing missing');
assert(/challenge:[\s\S]*?60/.test(engine),'challenge timing missing');
assert(/test:[\s\S]*?75/.test(engine),'final timing missing');
assert(nav.includes('const panoramaProse=['),'Panorama registry missing');
assert(nav.includes('The Gift of the Magi'),'Chapter 9 navigation title missing');
assert(nav.includes('if(n===5||n===6||n===7||n===8||n===9)'),'Ch5–Ch9 navigation handler missing');
assert(app.includes('EnglishPanoramaChapter1')&&app.includes('EnglishPanoramaChapter4'),'Ch1–Ch4 routes missing');
for(const m of ['EnglishPanoramaChapter5','EnglishPanoramaChapter6Final','EnglishPanoramaChapter7Final','EnglishPanoramaChapter8','EnglishPanoramaChapter9','n===12','n===13','n===14','n===15','n===16','if(chapter===9)'])assert(shell.includes(m),`shell routing missing: ${m}`);
...
assert(ch8.includes('THE PANORAMA • PROSE 8'),'Chapter 8 hero missing');
assert(ch9.includes('THE PANORAMA • PROSE 9'),'Chapter 9 hero missing');
assert(ch9.includes("const [mode,setMode]=useState(initialMode);const [quizOpen,setQuizOpen]=useState(initialMode!=='learn');"),'Chapter 9 mode state differs from Chapter 8');
assert(ch9.includes("const openMode=m=>{setMode(m);setQuizOpen(m!=='learn')};"),'Chapter 9 openMode differs from Chapter 8');
assert(ch9.includes("const bank=mode==='practice'?practice:mode==='challenge'?challenge:finalTest;"),'Chapter 9 bank selector differs from Chapter 8');
...
assert(ch9.includes('function StudyBlock({s})')&&ch9.includes('StudyBlock key={s.title} s={s}'),'Chapter 9 StudyBlock pattern differs from Chapter 8');
...
assert(ch9.includes('O. Henry')&&ch9.includes('Active and Passive Voice')&&ch9.includes('parsimony')&&ch9.includes('platinum fob chain'),'Chapter 9 content coverage missing');
assert(ch9.includes('composition:')&&ch9.includes('activities:')&&ch9.includes('translationPractice:'),'Chapter 9 supporting study sections missing');
