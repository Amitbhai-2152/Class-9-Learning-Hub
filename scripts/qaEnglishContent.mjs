...
assert(nav.includes("const proseMatch=chapter.match(/^Panorama • Prose"),'Panorama prose routing parser missing');
assert(nav.includes('[5,6,7,8,9].includes(prose)'),'Panorama routing handler must include Chapters 5–9');
assert(nav.includes("params.set(`panorama${prose}`,'1')"),'Panorama chapter routing must set the matching runtime flag');
