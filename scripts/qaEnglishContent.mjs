...
function bankChecks(text,name){
  ...
  assert(/setResult\(\{score:.*selected===current\.a\?1:0/.test(text),`${name}: final-question result scoring must include the selected answer`);
}
...
console.log('English content QA passed: book split, chapter registry, Reader/Prose Ch1 banks, deterministic answer distribution, actual scoring guard, guided study depth, word study, grammar, exam prep, and routing verified.');