...
for(const marker of ["mode==='practice'","mode==='challenge'","mode==='test'",'HINDI_MODE_TIMING','const ORDER='])if(!unified.includes(marker))failures.push(`missing unified learner marker: ${marker}`);
...
for(const title of redesigned){
  if(!unified.includes(`'${title}':{`))failures.push(`redesigned chapter missing from unified learner: ${title}`);
}
const factEntries=(text)=>{
  const start=text.indexOf('facts:[');
  if(start<0)return 0;
  const end=text.indexOf(']}\n};',start);
  const block=text.slice(start,end>start?end:text.length);
  return (block.match(/\['[^']+','/g)||[]).length;
};
const unifiedFactCount=(title)=>{
