...
const day12=recordXPActivity({at:'2026-09-12T10:00:00'});
assert.equal(day12.routine.currentStreak,3);
const rewardAt3=recordActivityAndRewards({at:'2026-09-13T10:00:00',previousXp:getXPState().totalXp,currentXp:getXPState().totalXp,subjectId:'math'});
assert.equal(rewardAt3.activity.routine.currentStreak,4);
assert.equal(rewardAt3.streakReward.awarded,0);
const summary=getRewardSummary();
assert.equal(summary.schemaVersion,1);
assert.equal(summary.levelProgress.level>=1,true);
assert.equal(summary.streak.best>=2,true);
...
console.log('XP Phase 3 QA passed: level progression, level reward idempotency, streak deduplication, missed-day reset, persistent best streak, reward traceability, and bounded bonus XP verified.');
