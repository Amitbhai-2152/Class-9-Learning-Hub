...
assert.match(boundary,/detail\.analytics\?\.lastAttemptAt\|\|detail\.lastActivityAt\|\|newAttemptId\(\)/);
assert.match(boundary,/const activityId=`hub:\$\{subjectId\}:\$\{topicId\}:stage:\$\{stage\}`/);
assert.match(boundary,/const stage=STAGES\.has\(String\(detail\.stage\|\|''\)\)\?String\(detail\.stage\):null/);
assert.doesNotMatch(boundary,/String\(detail\.topicId\|\|''\)!==context\.topicId/);
assert.doesNotMatch(boundary,/String\(detail\.subjectId\|\|''\)!==context\.subjectId/);
assert.doesNotMatch(boundary,/String\(detail\.stage\|\|''\)!==context\.stage/);
assert.doesNotMatch(boundary,/attemptId:newAttemptId\(\)/);
assert.doesNotMatch(boundary,/chapter===context\.topicId/);
...
console.log('XP Phase 4 QA passed: canonical dashboard state, global completion integration, normal chapter canonical test XP routing, deterministic attempt identity, reward persistence, homepage Daily Exam Plan placement, adaptive scoring, phase-aware recommendations, Test Centre planner contract, and mobile responsive safeguards verified.');
