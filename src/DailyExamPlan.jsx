...
const priorityLabel=item=>item.score>=120?'CRITICAL':item.score>=90?'HIGH':item.score>=65?'MEDIUM':'NORMAL';
...
export function DailyExamPlan(){
 ...
 return <section className="daily-exam-plan" aria-labelledby="daily-exam-plan-title">
  ...
  {plan.recommendations?.length>0&&<div className="daily-recommendations"><div className="daily-recommendations-head"><div><span>🧠 ADAPTIVE PRIORITY</span><strong>आज के लिए मेरी top recommendations</strong></div><small>{plan.highPriority?`${plan.highPriority} high-priority signal${plan.highPriority>1?'s':''}`:'Priority is recalculated from your current learning state'} • {plan.recommendationMode||'adaptive'}</small></div><div className="daily-recommendations-grid">{plan.recommendations.map((item,index)=>{const meta=getPlannerSubjectMeta(item.subjectId);const priority=priorityLabel(item);return <button type="button" className={`daily-recommendation priority-${priority.toLowerCase()}`} key={`${item.subjectId}:${item.topicId}`} onClick={()=>openTopic(item)}><span className="daily-recommendation-rank">{index+1}</span><span className="daily-recommendation-icon">{meta.icon}</span><span className="daily-recommendation-copy"><strong>{item.topic.title}</strong><small>{meta.name} • {item.reason}</small><em>{item.action} next</em></span><span className="daily-recommendation-priority">{priority}</span><span className="daily-recommendation-arrow">→</span></button>})}</div></div>}
