 ...
 return <div className="rlab-shell">
  ...
  <div className="rlab-content">
   ...
   <main className="rlab-main">
    ...
    {session&&testStarted&&<section className="rlab-quiz"><div className="quiz-top"><div><b>{mode==='test'?'FINAL TEST':mode==='practice'?'PRACTICE':'CHALLENGE'}</b><span>{Object.keys(answers).length} / {session.length} answered</span></div>{testStarted&&<div className={`countdown ${timeLeft<=60?'danger':''}`}>⏱️ {mm}:{ss}</div>}</div><div className="progressbar"><span style={{width:`${Math.min(100,Object.keys(answers).length/session.length*100)}%`}}/></div>{session.map((q,i)=><div className="question-card" key={`${q.question}-${i}`}><div className="q-meta">Q{i+1} <span>{q.difficulty}</span></div><h3>{q.question}</h3><div className="options">{q.options.map((o,j)=><button type="button" key={j} className={answers[i]?.text===o.text?'selected':''} onClick={()=>choose(i,o)}><span>{String.fromCharCode(65+j)}</span>{o.text}</button>)}</div>{answers[i]&&<div style={{marginTop:12,padding:'10px 12px',borderRadius:12,background:'#f7fafc'}}><b>✓ Answer recorded</b><div style={{marginTop:4}}>सही उत्तर, अंक और explanation results के अंत में दिखेंगे।</div></div>}</div>)}<div className="quiz-fallback-list">{session.every((_,i)=>answers[i]!==undefined)&&<div className="all-answered"><b>All questions answered.</b><button type="button" onClick={()=>finish(false)}>{mode==='test'?'Submit Final Test':'Finish & See Results'} →</button></div>}</div></section>}
    {completed&&<section className="rlab-result"><div className="result-score"><span>SCORE</span><strong>{score} / {mode==='test'?30:session?.length||20}</strong><p>{score>=Math.ceil((mode==='test'?30:20)*0.8)?'Excellent! तुम्हारी reasoning speed strong है.':score>=Math.ceil((mode==='test'?30:20)*0.6)?'Good work! थोड़ी और practice से accuracy बढ़ेगी.':'Practice जारी रखो और rules को step-by-step verify करो.'}</p></div><section className="answer-key" style={{marginTop:20}}><h3>📋 Answer Review</h3>{session.map((q,i)=>{const picked=answers[i];const ok=Boolean(picked&&picked.correct);return <article key={`${q.question}-${i}`} className="answer-key-item" style={{textAlign:'left',marginTop:10}}><strong>Q{i+1}</strong><div style={{marginTop:5}}>{q.question}</div><div style={{marginTop:5}}>Your answer: {picked?picked.text:'—'} {picked?(ok?'✓':'✗'):''}</div><div style={{marginTop:4}}>Correct answer: {q.correct}</div>{q.explanation&&<p style={{marginTop:5}}>💡 {q.explanation}</p>}</article>})}</section><button type="button" onClick={()=>begin(mode==='test'?'test':mode)}>Retake →</button></section>}
   </main>
  </div>
 </div>
}
