...
export default function ReasoningLabV2({initialChapter=null,initialMode=null,onExit=()=>{},addXp=()=>{},finishSession=()=>{}}){
 ...
 const [session,setSession]=useState(null);const [answers,setAnswers]=useState({});const [score,setScore]=useState(0);const [testStarted,setTestStarted]=useState(false);const [timeLeft,setTimeLeft]=useState(15*60);
 ...
 const begin=(nextMode)=>{setMode(nextMode);setAnswers({});setScore(0);if(nextMode==='test'){setSession(prepare(bank,counts.test));setTimeLeft(15*60);setTestStarted(false)}else{setSession(prepare(bank,counts[nextMode]));setTestStarted(true)}};
 const startTest=()=>{setAnswers({});setScore(0);setSession(prepare(bank,25));setTimeLeft(15*60);setTestStarted(true)};
 const finish=(timedOut=false)=>{if(!session)return;const correct=session.reduce((n,q,i)=>n+(answers[i]?.correct?1:0),0);setScore(correct);finishSession({subject:'reasoning',chapter:active.id,mode,correct,total:session.length,timedOut,at:new Date().toISOString()});addXp(correct);setTestStarted(false);if(timedOut)window.setTimeout(()=>{},0)};
 ...
 useEffect(()=>{setSession(null);setTestStarted(false);setAnswers({});setScore(0)},[chapterId]);
 ...
 return <div className="rlab-shell">
  ...
  <div className="rlab-content">
   ...
   <main className="rlab-main">
    ...
    {score>0||((session===null)&&answers&&Object.keys(answers).length===0&&mode==='test'&&timeLeft===0)?<section className="rlab-result"><div className="result-score"><span>SCORE</span><strong>{score} / {mode==='test'?25:score}</strong><p>{score>=Math.ceil((mode==='test'?25:15)*0.8)?'Excellent! तुम्हारी reasoning speed strong है.':score>=Math.ceil((mode==='test'?25:15)*0.6)?'Good work! थोड़ी और practice से accuracy बढ़ेगी.':'Practice जारी रखो और rules को step-by-step verify करो.'}</p></div><button type="button" onClick={()=>begin(mode==='test'?'test':mode)}>Retake →</button></section>:null}
   </main>
  </div>
 </div>
}