...
export default function ReasoningLabV2({initialChapter=null,initialMode=null,onExit=()=>{},addXp=()=>{},finishSession=()=>{}}){
 ...
 const[chapterId,setChapterId]=useState(chapterInitial);const[mode,setMode]=useState(modeInitial);const[session,setSession]=useState(null);const[answers,setAnswers]=useState({});const[score,setScore]=useState(0);const[completed,setCompleted]=useState(false);const[testStarted,setTestStarted]=useState(false);const[timeLeft,setTimeLeft]=useState(15*60);
 ...
 const begin=nextMode=>{setMode(nextMode);setAnswers({});setScore(0);setCompleted(false);if(nextMode==='test'){setSession(null);setTimeLeft(15*60);setTestStarted(false)}else{setSession(prepare(bank,counts[nextMode]));setTestStarted(true)}};
 const startTest=()=>{setAnswers({});setScore(0);setCompleted(false);setSession(prepare(bank,30));setTimeLeft(15*60);setTestStarted(true)};
 ...
 useEffect(()=>{if(mode!=='test'||!testStarted||!session)return;const id=setInterval(()=>{setTimeLeft(t=>{if(t<=1){clearInterval(id);finish(true);return 0}return t-1})},1000);return()=>clearInterval(id)},[mode,testStarted,session]);
 ...
 return <div className="rlab-shell">
  ...
  <div className="rlab-content">
   ...
   <main className="rlab-main">
    ...
    {(mode==='practice'||mode==='challenge')&&!session&&<section className="rlab-start"><div className="start-icon">{mode==='practice'?'✍️':'🔥'}</div><h2>{mode==='practice'?'Guided Practice':'Challenge Mode'}</h2><p>{mode==='practice'?'20 questions with instant explanation.':'20 higher-difficulty questions with mixed patterns.'}</p><button type="button" onClick={()=>begin(mode)}>Start {mode==='practice'?'Practice':'Challenge'} →</button></section>}
    {mode==='test'&&!testStarted&&<section className="rlab-start test-start"><div className="start-icon">🎯</div><div className="timer-preview">15:00</div><h2>Final Test</h2><p>30 प्रश्न • 15 मिनट • प्रत्येक attempt में options randomised होंगे।</p><div className="test-rules"><span>⏱️ Auto-submit on timeout</span><span>🔀 Random options</span><span>📊 Final score</span></div><button type="button" onClick={startTest}>Test शुरू करें →</button></section>}
    {session&&testStarted&&<section className="rlab-quiz"><div className="quiz-top"><div><b>{mode==='test'?'FINAL TEST':mode==='practice'?'PRACTICE':'CHALLENGE'}</b><span>{Object.keys(answers).length} / {session.length} answered</span></div>{mode==='test'&&<div className={`countdown ${timeLeft<=60?'danger':''}`}>⏱️ {mm}:{ss}</div>}</div><div className="progressbar"><span style={{width:`${Math.min(100,Object.keys(answers).length/session.length*100)}%`}}/></div>{session.map((q,i)=><div className="question-card" key={`${q.question}-${i}`}><div className="q-meta">Q{i+1} <span>{q.difficulty}</span></div><h3>{q.question}</h3><div className="options">{q.options.map((o,j)=><button type="button" key={j} className={answers[i]?.text===o.text?'selected':''} onClick={()=>choose(i,o)}><span>{String.fromCharCode(65+j)}</span>{o.text}</button>)}</div>{mode!=='test'&&answers[i]&&<div style={{marginTop:12,padding:'10px 12px',borderRadius:12,background:'#f7fafc'}}><b>{answers[i].correct?'✓ Correct':'✕ Review'}</b><div style={{marginTop:4}}>{q.explanation}</div></div>}</div>)}<div className="quiz-fallback-list">{session.every((_,i)=>answers[i]!==undefined)&&<div className="all-answered"><b>All questions answered.</b><button type="button" onClick={()=>finish(false)}>{mode==='test'?'Submit Final Test':'Finish & See Results'} →</button></div>}</div></section>}
