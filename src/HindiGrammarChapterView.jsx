...
export function HindiGrammarChapterView({topic,initialMode='learn',onBack,onComplete,onNavigate}){
  ...
  useEffect(()=>{
    if(!submitted||mode==='learn')return;
    onComplete?.(mode);
  },[submitted,mode,onComplete]);
  useEffect(()=>{
  ...
  if(mode==='learn')return <div className="hindi-learn hindi-chapter1-learn hindi-varnika-learn"><div className="hindi-learn-banner"><span>हिन्दी • व्याकरण एवं रचना • विषय {Math.max(topicIndex+1,1)}</span><h2>{data.title}</h2><p>{data.summary}</p></div><section className="hindi-ch1-panel"><div className="hindi-ch1-panel-head"><h3>📖 विषय को समझें</h3><span>{data.points.length} मुख्य बिंदु</span></div><div className="hindi-learn-grid">{data.points.map(([title,body],i)=><section key={`${title}-${i}`}><h3>{i+1}. {title}</h3><p>{body}</p></section>)}</div></section><div className="hindi-ch1-callout"><strong>📝 परीक्षा फोकस</strong><p>परिभाषा/नियम को उदाहरण के साथ समझें। अभ्यास में पहचान और प्रयोग दोनों पर ध्यान दें, तथा सामान्य भ्रम वाले विकल्पों को अलग करना सीखें।</p></div><div className="hindi-actions"><button type="button" className="secondary-btn pressable" onClick={onBack}>← व्याकरण सूची</button><div><button type="button" className="secondary-btn pressable" onClick={()=>setMode('practice')}>📝 अभ्यास शुरू करें</button><button type="button" className="primary-btn pressable" onClick={()=>onComplete?.('learn')}>✓ सीखना पूरा करें</button></div></div></div>;
