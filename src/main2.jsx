...
const KEEP_DEDICATED=new Set(['tenses','modals','voice','paragraph-essay','composition','translation']);
  ...
  useEffect(()=>{
    ...
    const checkOnce=async()=>{
      try{
        ...
        const bust=Date.now();
        const response=await fetch(`${base}build-version.json?hub_check=${bust}`,{cache:'no-store',headers:{'Cache-Control':'no-cache','Pragma':'no-cache'}});
