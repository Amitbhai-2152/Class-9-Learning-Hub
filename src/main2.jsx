...
function RootRouter(){const[route,setRoute]=useState(readRoute);useEffect(()=>{const sync=()=>setRoute(readRoute());window.addEventListener('popstate',sync);window.addEventListener('hashchange',sync);const timer=setInterval(sync,250);sync();return()=>{window.removeEventListener('popstate',sync);window.removeEventListener('hashchange',sync);clearInterval(timer)}},[]);
 ...
 if(route.languageSkills&&route.topic==='notice')return <EnglishNoticeWritingTopic {...sharedProps}/>;
 if(route.languageSkills&&route.topic==='report')return <EnglishReportWritingTopic {...sharedProps}/>;
 if(route.languageSkills&&route.topic==='speech')return route.mode==='learn'?<EnglishSpeechWritingTopic {...sharedProps}/>:<EnglishSpeechWritingAssessment mode={route.mode} onBack={exitLanguageSkills} {...sharedProps}/>;
 if(route.languageSkills&&route.topic==='message')return route.mode==='learn'?<EnglishMessageWritingTopic {...sharedProps}/>:<EnglishMessageWritingAssessment mode={route.mode} onBack={exitLanguageSkills} {...sharedProps}/>;
 if(route.languageSkills&&PHASE2_LANGUAGE_BANK_TOPICS.has(route.topic)&&route.mode!=='learn')return <EnglishLanguageSkillsPhase2Assessment topicId={route.topic} onBack={exitLanguageSkills} {...sharedProps}/>;
