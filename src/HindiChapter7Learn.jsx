...
export function HindiChapter7Learn({onBack,onModeComplete}){
 const lesson=hindiChapter7Lesson;
 return <div className="hindi-learn hindi-chapter1-learn">
  ...
  <section className="hindi-ch1-panel"><div className="hindi-ch1-panel-head"><h3>👥 प्रमुख व्यक्ति और संदर्भ</h3><span>कौन · भूमिका · महत्व</span></div><div className="hindi-ch1-characters">{lesson.people.map(([name,role,note])=><article key={name}><strong>{name}</strong><span>{role}</span><p>{note}</p></article>)}</div></section>
  ...
  <section className="hindi-ch1-panel"><div className="hindi-ch1-panel-head"><h3>🖊️ भाषा और साहित्यिक उपकरण</h3><span>रिपोर्ताज की शैली पकड़ें</span></div><div className="hindi-ch1-two-col">{lesson.literaryTools.map(([title,body])=><article key={title}><strong>{title}</strong><p>{body}</p></article>)}</div></section>
