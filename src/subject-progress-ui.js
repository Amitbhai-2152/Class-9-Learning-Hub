...
export function getSubjectProgress(){
 ...
 return Object.fromEntries(SUBJECT_REGISTRY.map(subject=>{
  ...
  return [subject.id,{id:subject.id,name:subject.name,total:subject.topics.length,completedStages,chapterStarted,fullyCompleted,attempts,correct,quizAttempts,questionsAnswered,questionsTotal,correctAnswers,accuracy,coverage,percent:readiness,readiness,updatedAt,topics:subject.topics.map(topic=>({id:topic.id,title:topic.title,order:topic.order,progress:Math.round((STAGES.filter(stage=>topics[canonicalTopicKey(subject.id,topic.id)]?.stages?.[stage]).length/STAGES.length)*100)}))}];
 }));
}
...
function render(){
 ...
 const meter=calculatePreparationMeter(canonical);
