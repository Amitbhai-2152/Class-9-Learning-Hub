...
export function HindiChapter10Learn({onBack,onModeComplete}){
  const lesson=hindiChapter10Lesson;
  return <div className="hindi-learn hindi-chapter1-learn">
    ...
    <div className="hindi-ch1-panel"><div className="hindi-ch1-panel-head"><h3>👤 लेखक परिचय</h3><span>लेखन-दृष्टि समझें</span></div><p>{lesson.authorNote}</p></div>
    ...
    <section className="hindi-ch1-panel"><div className="hindi-ch1-panel-head"><h3>⚖️ लेखन की तुलना</h3><span>रटी शैली बनाम रचनात्मक शैली</span></div><div className="hindi-ch1-two-col">{lesson.comparison.map(([title,nature,origin,style])=><article key={title}><strong>{title}</strong><p><b>दृष्टि:</b> {nature}</p><p><b>आधार:</b> {origin}</p><p><b>प्रभाव:</b> {style}</p></article>)}</div></section>
    <section className="hindi-ch1-panel"><div className="hindi-ch1-panel-head"><h3>🎯 मुख्य विचार</h3><span>दीर्घ उत्तरों में जोड़ें</span></div><div className="hindi-ch1-chip-grid">{lesson.themes.map(x=><span key={x}>{x}</span>)}</div></section>
    <section className="hindi-ch1-panel"><div className="hindi-ch1-panel-head"><h3>🖊️ लेखन की तकनीक</h3><span>निबंध को प्रभावी कैसे बनाएँ?</span></div><div className="hindi-ch1-two-col">{lesson.literaryTools.map(([title,body])=><article key={title}><strong>{title}</strong><p>{body}</p></article>)}</div></section>
