...
import {awardSmartXP,getXPState} from './engines/xp/xpStoreBridge.js';
...
const scoreFromRoot=root=>{
 const scoreNode=root?.querySelector('.result-score');
 const raw=textOf(scoreNode);
 ...
 const percentNode=root?.querySelector('.result-percent');
 const pm=textOf(percentNode).match(/(\d+(?:\.\d+)?)\s*%/);
...
export function XPCompletionBoundary({stage,activityId,subjectId=null,topicId=null,children,onAwarded}){
 ...
 const completionSerial=useRef(0);
