...
const storageKey='sst-history-ch8-progress';
const readProgress=()=>{try{return JSON.parse(localStorage.getItem(storageKey)||'{}')||{}}catch{return {}}};
const saveProgress=value=>{try{localStorage.setItem(storageKey,JSON.stringify(value));window.dispatchEvent(new Event('sst-progress-updated'))}catch{}};
const markComplete=()=>{try{const ids=JSON.parse(localStorage.getItem('sst-completed-chapters')||'[]')||[];if(!ids.includes('history-ch-8'))localStorage.setItem('sst-completed-chapters',JSON.stringify([...ids,'history-ch-8']));window.dispatchEvent(new Event('sst-progress-updated'))}catch{}};
const secondsPerQuestion=mode=>mode==='practice'?40:60;
const formatTime=seconds=>`${String(Math.floor(seconds/60)).padStart(2,'0')}:${String(seconds%60).padStart(2,'0')}`;
const MODES=[
{id:'learn',icon:'📖',title:'Learn',desc:'अध्याय को क्रम से समझें, उदाहरण जोड़ें और शब्दावली दोहराएँ।'},
{id:'practice',icon:'📝',title:'Practice',desc:'मुख्य अवधारणाओं पर बोर्ड-उपयोगी वस्तुनिष्ठ अभ्यास।'},
{id:'challenge',icon:'🔥',title:'Challenge',desc:'कारण–परिणाम, अनुप्रयोग और तर्क आधारित प्रश्न।'},
...
function LearnView({onComplete}){
 ...
 return <div className="history-learn">
  <div className="history-learn-nav" role="tablist" aria-label="अध्याय के पाठ">
   {data.lessons.map((item,index)=><button type="button" key={item.title} className={index===lesson?'is-active':''} onClick={()=>setLesson(index)}>{String(index+1).padStart(2,'0')}<span>{item.title}</span></button>)}
  </div>
  <article className="history-lesson-card">
   ...
   <div className="history-lesson-actions">
    <button type="button" className="history-secondary" disabled={lesson===0} onClick={()=>setLesson(value=>Math.max(0,value-1))}>← पिछला</button>
    {lesson===data.lessons.length-1?<button type="button" className="history-primary" onClick={()=>onComplete('learn',data.lessons.length,data.lessons.length)}>✓ Learn पूरा करें</button>:<button type="button" className="history-primary" onClick={()=>setLesson(value=>value+1)}>अगला →</button>}
   </div>
  </article>
  <section className="history-timeline-card">
   ...
   <div className="history-timeline">{data.timeline.map((item,index)=><div className="history-timeline-item" key={`${item[0]}-${index}`}><div className="history-timeline-dot">{index+1}</div><div><strong>{item[0]} · {item[1]}</strong><p>{item[2]}</p></div></div>)}</div>
  </section>
  <section className="history-terms-card">
   <button type="button" onClick={()=>setShowTerms(value=>!value)}><span>KEY TERMS</span><strong>मुख्य शब्दावली</strong><b>{showTerms?'−':'+'}</b></button>
...
function QuizView({mode,onComplete}){
 ...
 if(done)return <div className="history-result-card">
  ...
  <button type="button" className="history-primary" onClick={()=>{setIndex(0);setPicked(null);setScore(0);setDone(false);doneRef.current=false;setTimeLeft(questions.length*secondsPerQuestion(mode))}}>फिर से प्रयास करें</button>
 </div>;
