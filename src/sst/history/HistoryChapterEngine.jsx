...
function QuizView({data,mode,onComplete}){
 ...
 const scoreRef=useRef(score),pickedRef=useRef(picked),doneRef=useRef(done),completeRef=useRef(onComplete);
 ...
 const finish=()=>{if(doneRef.current)return;const finalScore=scoreRef.current+(pickedRef.current===q?.[2]?1:0);doneRef.current=true;setScore(finalScore);setDone(true);setTimeLeft(0);completeRef.current(mode,finalScore,questions.length)};
 ...
 const next=()=>{if(picked===null)return;const nextScore=score+(picked===q[2]?1:0);if(index<questions.length-1){setScore(nextScore);setIndex(v=>v+1);setPicked(null)}else{setScore(nextScore);doneRef.current=true;setDone(true);completeRef.current(mode,nextScore,questions.length)}};
 const restart=()=>{setIndex(0);setPicked(null);setScore(0);setDone(false);doneRef.current=false;setTimeLeft(questions.length*secondsPerQuestion(mode))};
 ...
 const answered=picked!==null;const totalSeconds=questions.length*secondsPerQuestion(mode);const timerClass=timeLeft<=Math.max(10,Math.floor(totalSeconds*.25))?'danger':timeLeft<=Math.floor(totalSeconds*.5)?'warning':'';
