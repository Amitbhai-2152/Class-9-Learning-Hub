...
const saveProgress=v=>{try{localStorage.setItem(storageKey,JSON.stringify(v));window.dispatchEvent(new Event('sst-progress-updated'))}catch{}};
const markComplete=()=>{try{const ids=JSON.parse(localStorage.getItem('sst-completed-chapters')||'[]')||[];if(!ids.includes('history-ch-7'))localStorage.setItem('sst-completed-chapters',JSON.stringify([...ids,'history-ch-7']));window.dispatchEvent(new Event('sst-progress-updated'))}catch{}};
...
{id:'subjective',icon:'✍️',title:'Subjective',desc:'लिखित उत्तर और बोर्ड-शैली अभ्यास।'}
];

function LearnView({onComplete}){
 const data=HISTORY_CHAPTER_7;
 const [lesson,setLesson]=useState(0);
 const [showTerms,setShowTerms]=useState(false);
 const current=data.lessons[lesson];
 return <div className="history-learn">
  <div className="history-learn-nav" role="tablist" aria-label="अध्याय के पाठ">
   {data.lessons.map((item,i)=><button key={item.title} className={i===lesson?'is-active':''} onClick={()=>setLesson(i)}>{String(i+1).padStart(2,'0')}<span>{item.title}</span></button>)}
  </div>
  <article className="history-lesson-card">
   ...
   <div className="history-point-grid">{current.points.map(point=><div key={point} className="history-point"><span>✓</span><p>{point}</p></div>)}</div>
   ...
   <div className="history-lesson-actions">
    <button className="history-secondary" disabled={lesson===0} onClick={()=>setLesson(v=>Math.max(0,v-1))}>← पिछला</button>
    {lesson===data.lessons.length-1?<button className="history-primary" onClick={()=>onComplete('learn',data.lessons.length,data.lessons.length)}>✓ Learn पूरा करें</button>:<button className="history-primary" onClick={()=>setLesson(v=>v+1)}>अगला →</button>}
   </div>
  </article>
  <section className="history-timeline-card">
   <div className="history-section-head"><div><span>QUICK TIMELINE</span><h3>मुख्य घटनाएँ एक नज़र में</h3></div><small>कालक्रम याद रखें</small></div>
  ...
  <section className="history-terms-card">
   <button onClick={()=>setShowTerms(v=>!v)}><span>KEY TERMS</span><strong>मुख्य शब्दावली</strong><b>{showTerms?'−':'+'}</b></button>
   {showTerms&&<div className="history-term-grid">{data.keyTerms.map(([term,meaning])=><div key={term}><strong>{term}</strong><p>{meaning}</p></div>)}</div>}
  </section>
 </div>;
}
...
function QuizView({mode,onComplete}){
 ...
 const [timeLeft,setTimeLeft]=useState(()=>questions.length*secondsPerQuestion(mode));
 ...
 useEffect(()=>{
  setIndex(0);setPicked(null);setScore(0);setDone(false);doneRef.current=false;
  setTimeLeft(questions.length*secondsPerQuestion(mode));
 ...
 useEffect(()=>{
  ...
  const timer=setInterval(()=>setTimeLeft(v=>{if(v<=1){clearInterval(timer);finish();return 0}return v-1}),1000);
 ...
 if(done)return <div className="history-result-card">
  ...
  <button className="history-primary" onClick={()=>{setIndex(0);setPicked(null);setScore(0);setDone(false);doneRef.current=false;setTimeLeft(questions.length*secondsPerQuestion(mode))}}>फिर से प्रयास करें</button>
 </div>;
...
function SubjectiveView(){
 const data=HISTORY_SUBJECTIVE_CHAPTER_7;
 const levels=[['easy','🌱 आसान','मूल अवधारणा और सीधे उत्तर'],['hard','🧠 कठिन','व्याख्या, कारण और संरचना'],['challenger','🏆 चैलेंजर','तुलना, मूल्यांकन और तर्क']];
 return <section className="sst-subjective-card" aria-labelledby="chapter7-subjective-title">
  <div className="sst-subjective-head"><div><span className="sst-subjective-kicker">✍ SUBJECTIVE PRACTICE</span><h2 id="chapter7-subjective-title">विषयपरक प्रश्न</h2><p>{data.title} • 12 प्रश्न • उत्तर अपने शब्दों में लिखें</p></div></div>
  <div className="sst-subjective-levels">
   {levels.map(([key,label,desc])=><div className={`sst-subjective-level level-${key}`} key={key}>
    ...
    <div className="sst-subjective-list">{data.questions[key].map((item,index)=><article className="sst-subjective-item" key={`${key}-${index}`}><div className="sst-subjective-number">{String(index+1).padStart(2,'0')}</div><div className="sst-subjective-question"><p>{item.q}</p><span>{item.marks} अंक</span></div></article>)}</div>
   </div>)}
  </div>
 </section>;
}

export function HistoryChapter7Engine({onBack}){
 const data=HISTORY_CHAPTER_7;
 const [mode,setMode]=useState(null);
 const [progress,setProgress]=useState(readProgress);
 useEffect(()=>setProgress(readProgress()),[]);
 const completion=['learn','practice','challenge','test'].filter(id=>progress[id]).length;
 ...
 return <main className="history-chapter">
  <header className="history-chapter-header">
   ...
   <div className="history-header-copy"><span>इतिहास • अध्याय 7 • बिहार बोर्ड</span><h1>{data.title}</h1><p>{data.subtitle}</p>{progress.chapterCompleted&&<div className="history-complete-badge">✓ CHAPTER COMPLETED</div>}</div>
   <div className="history-stat-card"><strong>{completion}/4</strong><span>stages complete</span></div>
  </header>
