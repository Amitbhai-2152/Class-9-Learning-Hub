...
export default function SSTRoot(){
  ...
  if(route.track==='history'&&Number(route.chapter)===1){return <HistoryChapterEngine onBack={backToSST}/>}
  if(route.track&&route.chapter){const chapter=findSSTChapter(route.track,Number(route.chapter));if(chapter)return <main className="page"><header className="page-header sst-page-header"><button type="button" onClick={backToSST}>← सामाजिक विज्ञान</button><span>{chapter.trackName} • अध्याय {chapter.chapterNumber}</span><h1>{chapter.title}</h1><p>इस अध्याय का dedicated learning module जल्द उपलब्ध होगा।</p></header><section className="page-content"><div className="sst-coming-shell"><strong>Module queued</strong><p>Foundation, routing और chapter registry तैयार हैं। यह chapter अगले content phase में Learn → Practice → Challenge → Test engine से जोड़ा जाएगा।</p></div></section></main>}
