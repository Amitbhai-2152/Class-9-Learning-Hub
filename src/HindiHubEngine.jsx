...
import {getHindiProgress,isHindiChapterCompleted,isHindiModeCompleted,markHindiChapterCompleted,markHindiModeCompleted} from './hindiChapterProgress';
...
function makeLiteratureQuestions(data,challenge){
  ...
  const questions=[
    ...
    {q:`निम्न में से कौन-सा शब्द “${data.title}” से सीधे जुड़ा है?`,options:keywords,answer:0,explain:`“${keywords[0]}” इस पाठ के मुख्य keywords में है।`}
  ];
...
function shuffleQuestions(list){
  return list.map(q=>{
    const mixed=q.options.map((text,index)=>({text,index}));
    for(let i=mixed.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[mixed[i],mixed[j]]=[mixed[j],mixed[i]];}
    return {...q,options:mixed.map(x=>x.text),answer:mixed.findIndex(x=>x.index===q.answer)};
  });
}

function QuestionAssessment({questions,mode,onBack,addXp,finishSession,data,onModeComplete}){
  const[answers,setAnswers]=useState({});const[index,setIndex]=useState(0);const[result,setResult]=useState(null);const[time,setTime]=useState(mode==='test'?180:null);const finishedRef=useRef(false);
  const finish=(finalAnswers)=>{
    if(finishedRef.current)return;
    finishedRef.current=true;
    const review=questions.map((q,i)=>({question:q.q,options:q.options,selected:finalAnswers[i],answer:q.answer,correct:finalAnswers[i]===q.answer,explain:q.explain}));
    const correct=review.filter(x=>x.correct).length;const attempted=review.filter(x=>x.selected!=null).length;const earned=correct*(mode==='challenge'?5:3);
    onModeComplete?.(mode);addXp?.(earned);finishSession?.({subject:data.book||'गोधूली भाग 1',chapter:data.title,mode,attempted,correct,completed:true,earned,date:new Date().toISOString()});setResult({review,correct,attempted,earned});
  };
  ...
  useEffect(()=>{if(mode==='test'&&time===0&&!result)finish(answers)},[time,result,answers]);
...
export function HindiHubEngine({chapter,onBack,addXp,finishSession,initialMode=null}){
  ...
  const[mode,setMode]=useState(initialMode);const[,force]=useState(0);
  useEffect(()=>{setMode(initialMode)},[initialMode,chapter]);
  ...
  const questions=useMemo(()=>{const raw=isChapter1?(mode==='challenge'?hindiChapter1Challenge:hindiChapter1Questions):(grammarBank[data.title]?makeGrammarQuestions(data):makeLiteratureQuestions(data,mode==='challenge'));return shuffleQuestions(raw);},[data,mode,isChapter1]);
  const modeComplete=modeName=>{if(!data.id)return;const was=isHindiModeCompleted(data.id,modeName);markHindiModeCompleted(data.id,modeName);if(!was&&modeName==='learn')addXp?.(20);force(x=>x+1);};
  const exitMode=()=>setMode(null);
  if(mode==='learn')return isChapter1?<Chapter1Learn onBack={exitMode} onModeComplete={modeComplete}/>:<LearnView data={data} onBack={exitMode} addXp={addXp} onModeComplete={modeComplete}/>;
  if(['practice','challenge','test'].includes(mode))return <QuestionAssessment questions={questions} data={data} mode={mode} onBack={exitMode} addXp={addXp} finishSession={finishSession} onModeComplete={modeComplete}/>;
