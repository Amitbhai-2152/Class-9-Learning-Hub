...
export function LearningEngine({subject,chapter,data,onBack,addXp,finishSession}){
  ...
  useEffect(()=>{
    try{
      ...
      if(raw){
        ...
        if(Number.isInteger(state.index))setIndex(Math.min(Math.max(state.index,0),Math.max(lessons.length-1,0)));
  ...
  const goNext=()=>{
    ...
    if(!completed){
      ...
      addXp?.(20);
      finishSession?.({subject:subject?.name,chapter,mode:'learn',completed:true,attempted:lessons.filter(x=>x.question).length,correct:correctCount,at:Date.now(),durationSeconds:Math.round((Date.now()-startedAt)/1000)});
      return;
    }
    onBack();
  };
