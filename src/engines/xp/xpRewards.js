...
export const XP_REWARDS_SCHEMA_VERSION=1;
...
const defaultRoutine=()=>({schemaVersion:XP_REWARDS_SCHEMA_VERSION,currentStreak:0,bestStreak:0,lastActiveDay:null,activeDays:0,claimedLevelRewards:[],claimedStreakRewards:[],updatedAt:null});
...
export function getXPRoutine(){
 ...
 return {
  ...
  activeDays:Math.max(0,int(raw.activeDays)),
  ...
  lastActiveDay:parseDay(raw.lastActiveDay)?raw.lastActiveDay:null,
...
export function recordXPActivity({at=null}={}){
 ...
 if(routine.lastActiveDay===today)return {updated:false,duplicate:true,routine};
 ...
 const next={...routine,currentStreak:nextStreak,bestStreak:Math.max(routine.bestStreak,nextStreak),lastActiveDay:today,activeDays:routine.activeDays+1,updatedAt:new Date().toISOString()};
...
export function getRewardSummary(){
 ...
 const routine=getXPRoutine();
 return {schemaVersion:XP_REWARDS_SCHEMA_VERSION,xp:state.totalXp,levelProgress:calculateLevelProgress(state.totalXp),streak:{current:routine.currentStreak,best:routine.bestStreak,lastActiveDay:routine.lastActiveDay,activeDays:routine.activeDays},claimedLevelRewards:routine.claimedLevelRewards.slice(),claimedStreakRewards:routine.claimedStreakRewards.slice(),ledgerRewards:getXPLedger().filter(e=>e.source==='bonus').length};
}
