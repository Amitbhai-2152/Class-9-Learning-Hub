...
export function restoreXPRoutine(routine){
 const raw={...defaultRoutine(),...safeObject(routine)};
 const history=validHistory(raw.activeDayHistory);
 const last=history.length?history[history.length-1]:(parseDay(raw.lastActiveDay)?String(raw.lastActiveDay):null);
 const restored={
  ...raw,
  schemaVersion:XP_REWARDS_SCHEMA_VERSION,
  currentStreak:history.length?consecutiveStreak(history):Math.max(0,int(raw.currentStreak)),
  bestStreak:Math.max(0,int(raw.bestStreak),history.length?consecutiveStreak(history):0),
  lastActiveDay:last,
  activeDays:Math.max(Math.max(0,int(raw.activeDays)),history.length),
  activeDayHistory:history,
  claimedLevelRewards:Array.from(new Set((Array.isArray(raw.claimedLevelRewards)?raw.claimedLevelRewards:[]).map(int).filter(v=>v>0))).sort((a,b)=>a-b),
  claimedStreakRewards:Array.from(new Set((Array.isArray(raw.claimedStreakRewards)?raw.claimedStreakRewards:[]).map(int).filter(v=>v>0))).sort((a,b)=>a-b),
  updatedAt:raw.updatedAt||null,
 };
 writeJson(XP_ROUTINE_KEY,restored);
 return restored;
}
