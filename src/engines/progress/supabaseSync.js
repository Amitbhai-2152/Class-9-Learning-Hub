...
import {getXPRoutine,mergeXPRoutines,restoreXPRoutine}from '../xp/xpRewards.js';
...
function mergeCanonical(localState,cloudRows,cloudAttempts){
 ...
 for(const row of cloudRows||[]){const key=`${row.subject_id}::${row.topic_id}`;const merged=mergeTopic(safeObject(topics[key]),row);const ids=new Set(merged.analytics.attemptIds||[]);const records=new Map((merged.analytics.attemptRecords||[]).map(a=>[String(a.attemptId),a]));for(const a of attemptsByTopic.get(key)||[]){if(a.attempt_key)ids.add(String(a.attempt_key));if(a.attempt_key)records.set(String(a.attempt_key),{attemptId:String(a.attempt_key),questionsAnswered:safeNum(a.questions_answered),questionsTotal:safeNum(a.questions_total),correctAnswers:safeNum(a.correct_answers),percent:safePercent(a.percent),at:iso(a.attempted_at)||a.attempted_at||null})}merged.analytics.attemptIds=Array.from(ids).slice(-100);merged.analytics.attemptRecords=Array.from(records.values()).slice(-100);merged.analytics.quizAttempts=Math.max(merged.analytics.quizAttempts,merged.analytics.attemptRecords.length);topics[key]=merged;}
...
async function ensureStudent(user){const payload={auth_user_id:user.id,anonymous_student_id:`auth:${user.id}`,full_name:String(user.user_metadata?.full_name||'').trim()||null,email:String(user.email||'').trim()||null,class_label:'9',schema_version:1,updated_at:new Date().toISOString()};const {data,error}=await supabase.from('students').upsert(payload,{onConflict:'auth_user_id'}).select('id,auth_user_id,anonymous_student_id').single();if(error)throw error;return data;}

async function pullCloud(studentId){const [progress,attempts,xpWallet,xpEvents]=await Promise.all([supabase.from('topic_progress').select('*').eq('student_id',studentId),supabase.from('quiz_attempts').select('*').eq('student_id',studentId).order('attempted_at',{ascending:false}),supabase.from('xp_wallets').select('*').eq('student_id',studentId).maybeSingle(),supabase.from('xp_events').select('*').eq('student_id',studentId).order('awarded_at',{ascending:false}).limit(1000)]);for(const result of [progress,attempts,xpWallet,xpEvents])if(result.error)throw result.error;return {progressRows:progress.data||[],attempts:attempts.data||[],xpWallet:xpWallet.data||null,xpEvents:xpEvents.data||[]};}

async function pushProgress(studentId,state){
 const rows=[],attemptRows=[];Object.entries(safeObject(state?.topics)).forEach(([key,row])=>{const [subjectId,topicId]=key.split('::');const stages=safeObject(row?.stages),a=safeObject(row?.analytics);if(!subjectId||!topicId)return;rows.push({student_id:studentId,subject_id:subjectId,topic_id:topicId,learn_complete:Boolean(stages.learn),practice_complete:Boolean(stages.practice),challenge_complete:Boolean(stages.challenge),test_complete:Boolean(stages.test),attempts:Math.max(0,safeNum(row?.attempts)),correct:Math.max(0,safeNum(row?.correct)),quiz_attempts:Math.max(0,safeNum(a.quizAttempts)),questions_answered:Math.max(0,safeNum(a.questionsAnswered)),questions_total:Math.max(0,safeNum(a.questionsTotal)),correct_answers:Math.max(0,safeNum(a.correctAnswers)),best_percent:safePercent(a.bestPercent),last_percent:safePercent(a.lastPercent),last_attempt_at:iso(a.lastAttemptAt),last_activity_at:iso(row?.lastActivityAt),updated_at:new Date().toISOString()});for(const attempt of Array.isArray(a.attemptRecords)?a.attemptRecords:[]){const answered=Math.max(0,safeNum(attempt.questionsAnswered));const total=Math.max(answered,safeNum(attempt.questionsTotal));const correct=Math.max(0,Math.min(answered,safeNum(attempt.correctAnswers)));attemptRows.push({student_id:studentId,subject_id:subjectId,topic_id:topicId,stage:String(row?.lastStage||'practice'),attempt_key:String(attempt.attemptId||''),questions_answered:answered,questions_total:total,correct_answers:correct,percent:safePercent(attempt.percent),attempted_at:iso(attempt.at)||new Date().toISOString()});}});
 ...
 if(attemptRows.length){const sanitized=attemptRows.filter(r=>r.attempt_key.length>0&&['practice','challenge','test'].includes(r.stage));if(sanitized.length){const {error}=await supabase.from('quiz_attempts').upsert(sanitized,{onConflict:'student_id,attempt_key',ignoreDuplicates:true});if(error)throw error;}}
}

async function pushXP(studentId,routine=getXPRoutine()){
 ...
 snapshot.wallet={...snapshot.wallet,
  ...
  claimed_level_rewards:Array.isArray(routine.claimedLevelRewards)?routine.claimedLevelRewards:[],
  claimed_streak_rewards:Array.isArray(routine.claimedStreakRewards)?routine.claimedStreakRewards:[],
  updated_at:new Date().toISOString()
 };
 const {error:walletError}=await supabase.from('xp_wallets').upsert({student_id:studentId,...snapshot.wallet},{onConflict:'student_id'});if(walletError)throw walletError;
