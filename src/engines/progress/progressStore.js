...
function touchTopic({subjectRecord,topic,stage,at,attempts=0,correct=0,quiz=null}){
 ...
 const previousAnalytics=safeObject(previous.analytics);const previousAttemptIds=Array.isArray(previousAnalytics.attemptIds)?previousAnalytics.attemptIds:[];const previousAttemptRecords=Array.isArray(previousAnalytics.attemptRecords)?previousAnalytics.attemptRecords:[];
 let analytics={quizAttempts:safeNumber(previousAnalytics.quizAttempts),questionsAnswered:safeNumber(previousAnalytics.questionsAnswered),questionsTotal:safeNumber(previousAnalytics.questionsTotal),correctAnswers:safeNumber(previousAnalytics.correctAnswers),bestPercent:safePercent(previousAnalytics.bestPercent),lastPercent:safePercent(previousAnalytics.lastPercent),lastAttemptAt:previousAnalytics.lastAttemptAt||null,attemptIds:previousAttemptIds,attemptRecords:previousAttemptRecords};
 if(quiz&&quiz.attemptId){
  const attemptId=String(quiz.attemptId);
  if(!previousAttemptIds.includes(attemptId)){
   const questionsAnswered=Math.max(0,safeNumber(quiz.questionsAnswered));const questionsTotal=Math.max(questionsAnswered,safeNumber(quiz.questionsTotal));const correctAnswers=Math.max(0,Math.min(questionsAnswered,safeNumber(quiz.correctAnswers)));const percent=questionsAnswered?Math.round((correctAnswers/questionsAnswered)*100):safePercent(quiz.percent);const attemptedAt=quiz.at&&Number.isFinite(Date.parse(quiz.at))?new Date(quiz.at).toISOString():now;
   analytics={...analytics,quizAttempts:analytics.quizAttempts+1,questionsAnswered:analytics.questionsAnswered+questionsAnswered,questionsTotal:analytics.questionsTotal+questionsTotal,correctAnswers:analytics.correctAnswers+correctAnswers,bestPercent:Math.max(analytics.bestPercent,percent),lastPercent:percent,lastAttemptAt:attemptedAt,attemptIds:[...previousAttemptIds,attemptId].slice(-100),attemptRecords:[...previousAttemptRecords,{attemptId,questionsAnswered,questionsTotal,correctAnswers,percent,at:attemptedAt}].slice(-100)};
  }
 }
...
function sessionIdentity(session){const explicit=session?.attemptId||session?.id||session?.sessionId;if(explicit)return String(explicit);return [session?.at??'',session?.subject??'',session?.chapter??'',session?.mode??'',session?.score??session?.correct??'',session?.total??'',session?.percent??'',session?.attempted??''].join('|');}
...
function migrateLegacyProgress(){
 const state=readCanonical();const migration=safeObject(state.migrations);if(migration.legacyV1===true)return state;const generic=safeObject(readLegacy(KEY,{}));Object.entries(generic).forEach(([key,value])=>{const parts=String(key).split('::');const subject=parts.shift()||'';const chapter=parts.join('::');const row=safeObject(value);STAGES.forEach(stage=>{if(row[stage])recordCanonicalStage({subject,chapter,stage,attempts:safeNumber(row.attempts),correct:safeNumber(row.correct),at:row.lastActivityAt||new Date().toISOString()});});});
 const sessions=[];const app=readLegacy(APP_KEY,{});if(Array.isArray(app?.sessions))sessions.push(app.sessions);const engine=readLegacy('class9-sessions',[]);if(Array.isArray(engine))sessions.push(engine);const seen=new Set();sessions.forEach(items=>items.forEach(session=>{if(!session?.completed)return;const stage=STAGES.includes(session.mode)?session.mode:null;if(!stage||!session.subject||session.chapter==null)return;const attemptId=sessionIdentity(session);if(seen.has(attemptId))return;seen.add(attemptId);const correctAnswers=safeNumber(session.correct??session.score);const questionsTotal=Math.max(safeNumber(session.total),correctAnswers);const questionsAnswered=Math.max(0,Math.min(questionsTotal,safeNumber(session.attempted??session.total)));const percent=questionsAnswered?Math.round((correctAnswers/questionsAnswered)*100):safePercent(session.percent);recordCanonicalQuizAttempt({subject:session.subject,chapter:session.chapter,stage,attemptId,questionsAnswered,questionsTotal,correctAnswers,percent,at:session.at?new Date(session.at).toISOString():new Date().toISOString()});}));
 const hindi=safeObject(readLegacy('class9-hindi-chapter-progress-v1',{}));Object.entries(safeObject(hindi.modes)).forEach(([id,modes])=>Object.entries(safeObject(modes)).forEach(([stage,done])=>{if(done)recordCanonicalStage({subject:'हिन्दी',chapter:id,stage,at:new Date().toISOString()});}));const migrated=readCanonical();const now=new Date().toISOString();const finalState={schemaVersion:1,studentId:migrated.studentId||getStudentId(),updatedAt:migrated.updatedAt||now,topics:safeObject(migrated.topics),migrations:{...safeObject(migrated.migrations),legacyV1:true,legacyV1At:now}};writeCanonical(finalState);return finalState;
}
...
migrateAppProgress();migrateLegacyProgress();
