...
export function ScienceChapter4Engine({chapter,onBack,addXp,finishSession}){
 const modeDefault='practice';const [mode,setMode]=useState(null);const [index,setIndex]=useState(0);const [selected,setSelected]=useState(null);const [score,setScore]=useState(0);const [done,setDone]=useState(false);const [items,setItems]=useState([]);
 const questions=useMemo(()=>mode==='test'?shuffle(bank).slice(0,20):mode==='challenge'?shuffle(bank).slice(0,12):bank.slice(0,15),[mode]);
 useEffect(()=>{if(!mode)return;setItems(questions);setIndex(0);setSelected(null);setScore(0);setDone(false)},[mode]);
 const current=items[index];
 const choose=i=>{if(selected!==null)return;setSelected(i);if(i===current[2]){setScore(s=>s+1);addXp?.(mode==='challenge'?25:15)}};
 const next=()=>{if(index<items.length-1){setIndex(i=>i+1);setSelected(null)}else{setDone(true);markStageComplete(`विज्ञान::${chapter}`,mode);finishSession?.({subject:'विज्ञान',chapter,mode,attempted:items.length,correct:score,completed:true,at:Date.now()})}};
