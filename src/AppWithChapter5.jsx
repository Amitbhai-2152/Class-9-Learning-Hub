...
const safeProgress=value=>{const source=value&&typeof value==='object'&&!Array.isArray(value)?value:{};return {...initial,...source,xp:Number.isFinite(source.xp)?source.xp:0,streak:Number.isFinite(source.streak)?source.streak:1,dailyXp:Number.isFinite(source.dailyXp)?Math.min(source.goal,source.dailyXp):0,goal:Number.isFinite(source.goal)&&source.goal>0?source.goal:100,sessions:Array.isArray(source.sessions)?source.sessions:[]}};
...
const getChapter=()=>{if(typeof window==='undefined')return 0;const p=new URLSearchParams(window.location.search);if(p.get('subject')!=='english')return 0;if(p.get('languageSkills')==='1')return 11;if(p.get('proseRevision')==='1')return 10;if(p.get('panoramaPoetry7')==='1')return 23;if(p.get('panoramaPoetry6')==='1')return 22;if(p.get('panoramaPoetry5')==='1')return 21;if(p.get('panoramaPoetry4')==='1')return 20;if(p.get('panoramaPoetry3')==='1')return 19;if(p.get('panoramaPoetry2')==='1')return 18;if(p.get('panoramaPoetry1')==='1')return 17;if(p.get('panorama9')==='1')return 9;if(p.get('panorama8')==='1')return 8;if(p.get('panorama7')==='1')return 7;if(p.get('panorama6')==='1')return 6;if(p.get('panorama5')==='1')return 5;if(p.get('page')==='chapter'){const n=Number(p.get('chapter'));if(Number.isInteger(n)&&n===12)return 5;if(Number.isInteger(n)&&n===13)return 6;if(Number.isInteger(n)&&n===14)return 7;if(Number.isInteger(n)&&n===15)return 8;if(Number.isInteger(n)&&n===16)return 9;if(Number.isInteger(n)&&n===17)return 17;if(Number.isInteger(n)&&n===18)return 18;if(Number.isInteger(n)&&n===19)return 19;if(Number.isInteger(n)&&n===20)return 20;if(Number.isInteger(n)&&n===21)return 21;if(Number.isInteger(n)&&n===22)return 22;if(Number.isInteger(n)&&n===23)return 23}return 0};
...
export default function AppWithChapter5(){
  ...
  if(chapter===23){const mode=routeMode();return <EnglishPanoramaPoem7WithGuide initialMode={mode==='learn'?'':mode} onBack={back} addXp={addXp} finishSession={finishSession}/>;}
  if(chapter===22){const mode=routeMode();return <EnglishPanoramaPoem6 initialMode={mode==='learn'?'':mode} onBack={back} addXp={addXp} finishSession={finishSession}/>;}
  if(chapter===21){const mode=routeMode();return <EnglishPanoramaPoem5 initialMode={mode} onBack={back} addXp={addXp} finishSession={finishSession}/>;}
