...
const isChapter5Route=()=>{if(typeof window==='undefined')return false;const p=new URLSearchParams(window.location.search);return p.get('panorama5')==='1'&&p.get('subject')==='english'};
...
export default function AppWithChapter5(){
  const[chapter5,setChapter5]=useState(isChapter5Route);
  const[progress,setProgress]=useState(loadProgress);
  useEffect(()=>{
    try{localStorage.setItem('class9-progress',JSON.stringify(safeProgress(progress)))}catch{}
  },[progress]);
  useEffect(()=>{
    const sync=()=>setChapter5(isChapter5Route());
  ...
  const back=()=>{
    ...
    params.set('page','subject');params.set('subject','english');
    window.history.pushState({},'',`${window.location.pathname}?${params.toString()}${window.location.hash||''}`);
    setChapter5(false);
  };
  if(chapter5)return <EnglishPanoramaChapter5 initialMode={routeMode()} onBack={back} addXp={addXp} finishSession={finishSession}/>;
  return <App/>;
}
