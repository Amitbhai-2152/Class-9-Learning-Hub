...
function App(){
  ...
  useEffect(()=>{
    const syncFromUrl=()=>{
      const next=routeState(routeFromLocation());
      setPage(next.page);setSubject(next.subject);setChapter(next.chapter);setInitialMode(next.initialMode);
    };
    window.addEventListener('popstate',syncFromUrl);
    return()=>window.removeEventListener('popstate',syncFromUrl);
  },[]);
  const addXp=n=>{const amount=Number.isFinite(n)?n:0;setProgress(p=>{const safe=safeProgress(p);return {...safe,xp:safe.xp+amount,dailyXp:Math.min(safe.goal,safe.dailyXp+amount)}});if(amount){setToast(`+${amount} XP`);setTimeout(()=>setToast(''),1100)}};
  const finishSession=meta=>setProgress(p=>{const safe=safeProgress(p);return {...safe,sessions:[...safe.sessions,meta].slice(-100)}});
  const navigatePage=nextPage=>{
    const next={page:nextPage,subjectId:null,chapterIndex:null,mode:null};
    setPage(nextPage);setSubject(null);setChapter(null);setInitialMode(null);writeRoute(next);
  };
  const startSubject=s=>{
    setSubject(s);setChapter(null);setInitialMode(null);setPage('subject');
    writeRoute({page:'subject',subjectId:s.id,chapterIndex:null,mode:null});
  };
  const startChapter=(c,mode=null)=>{
    const chapterIndex=subject?.chapters.indexOf(c)??-1;
    if(chapterIndex<0||!subject)return;
    setChapter(c);setInitialMode(mode);setPage('chapter');
    writeRoute({page:'chapter',subjectId:subject.id,chapterIndex,mode});
  };
  const backToSubject=()=>{
    if(!subject)return navigatePage('classes');
    setChapter(null);setInitialMode(null);setPage('subject');
    writeRoute({page:'subject',subjectId:subject.id,chapterIndex:null,mode:null});
  };
  const onChapterModeChange=mode=>{
    setInitialMode(mode);
    writeRoute({page:'chapter',subjectId:subject?.id||null,chapterIndex:subject&&chapter?subject.chapters.indexOf(chapter):null,mode});
  };
...
function Meter({progress,back}){
  ...
  const metrics=calculatePreparationMeter(canonical);
  const subjectMap=new Map(metrics.subjectBreakdown.map(row=>[row.subjectId,row]));
  const subjectRows=subjects.map(subject=>({...subject,...(subjectMap.get(subject.id)||{coveragePercent:0,performancePercent:0,readiness:0,completedStages:0,totalStages:subject.chapters.length*4,topicsStarted:0,totalTopics:subject.chapters.length,quizAttempts:0,questionsAnswered:0,correctAnswers:0,hasPerformanceData:false})}));
  const ranked=[...subjectRows].sort((a,b)=>a.readiness-b.readiness);
  const weakest=ranked.slice(0,2);
  const action=metrics.totalTopics===0?'पहले किसी अध्याय में पढ़ाई शुरू करें।':metrics.coveragePercent<25?'पहले अधिक chapters में Learn पूरा करें।':metrics.performancePercent<70?'अब Practice और Challenge में accuracy बढ़ाएँ।':metrics.coveragePercent<75?'अधिक chapters के चारों stages पूरे करें।':'Test mode में अपनी तैयारी को validate करें।';
  const actionDetail=metrics.coveragePercent<25?'Coverage सबसे पहले बढ़ाने से readiness जल्दी मजबूत होगी।':metrics.performancePercent<70?'Quiz performance अभी readiness का सबसे बड़ा improvement area है।':'Coverage और quiz performance दोनों मजबूत हो रहे हैं।';
  const scoreStyle={fontVariantNumeric:'tabular-nums'};
  return <Simple title="तैयारी मीटर" back={back}><div className="meter-box" style={{padding:24}}><div style={{display:'grid',gridTemplateColumns:'minmax(180px,240px) 1fr',gap:24,alignItems:'center',textAlign:'left'}}><div style={{textAlign:'center'}}><div className="meter-ring" style={{'--meter':`${metrics.readiness}%`,'--meter-angle':`${metrics.readiness*3.6}deg`}}><span style={scoreStyle}>{metrics.readiness}%</span></div><div style={{fontWeight:800,fontSize:'1.05rem'}}>{metrics.label}</div></div><div><div style={{fontSize:'.72rem',fontWeight:800,letterSpacing:'.08em',color:'#737d8d',textTransform:'uppercase'}}>Readiness Score</div><h2 style={{margin:'7px 0 8px',fontSize:'1.65rem'}}>आपकी तैयारी का वास्तविक snapshot</h2><p style={{color:'#697383',lineHeight:1.6,margin:'0 0 18px'}}>Score canonical learning stages और वास्तविक quiz analytics पर आधारित है। XP इस score को प्रभावित नहीं करता।</p><div style={{display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))',gap:10}}><div style={{background:'#f4f5fa',borderRadius:12,padding:12}}><small style={{display:'block',color:'#747d8a'}}>Coverage</small><strong style={{fontSize:'1.1rem'}}>{metrics.coveragePercent}%</strong></div><div style={{background:'#f4f5fa',borderRadius:12,padding:12}}><small style={{display:'block',color:'#747d8a'}}>Quiz accuracy</small><strong style={{fontSize:'1.1rem'}}>{metrics.hasPerformanceData?`${metrics.performancePercent}%`:'—'}</strong></div><div style={{background:'#f4f5fa',borderRadius:12,padding:12}}><small style={{display:'block',color:'#747d8a'}}>Stages</small><strong style={{fontSize:'1.1rem'}}>{metrics.completedStages}/{metrics.totalStages}</strong></div></div></div></div></div><div style={{marginTop:24,display:'grid',gridTemplateColumns:'1.15fr .85fr',gap:16}}><section style={{background:'#fff',border:'1px solid #e7e9ef',borderRadius:16,padding:18}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,marginBottom:14}}><div><div style={{fontSize:'.72rem',fontWeight:800,letterSpacing:'.08em',color:'#737d8d'}}>SUBJECT READINESS</div><h3 style={{margin:'5px 0 0',fontSize:'1.2rem'}}>सातों विषयों की स्थिति</h3></div><span style={{fontSize:'.72rem',fontWeight:750,color:'#687181',background:'#f1f3f7',borderRadius:999,padding:'6px 9px'}}>Live</span></div><div style={{display:'grid',gap:9}}>{subjectRows.map(row=><div key={row.id} style={{border:'1px solid #edf0f4',borderRadius:13,padding:'10px 12px'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:10}}><div style={{display:'flex',alignItems:'center',gap:9,minWidth:0}}><span style={{width:32,height:32,borderRadius:9,background:'#eef0ff',display:'grid',placeItems:'center',flex:'0 0 auto'}}>{row.icon}</span><div style={{minWidth:0}}><strong style={{display:'block',fontSize:'.9rem'}}>{row.name}</strong><small style={{color:'#7a8290'}}>{row.topicsStarted}/{row.totalTopics} topics started • {row.completedStages}/{row.totalStages} stages</small></div></div><strong style={{fontSize:'1rem'}}>{row.readiness}%</strong></div><div style={{height:6,background:'#eceef3',borderRadius:999,overflow:'hidden',marginTop:8}}><span style={{display:'block',height:'100%',width:`${row.readiness}%`,background:'linear-gradient(90deg,#6674ee,#9a8cff)',borderRadius:'inherit'}}/></div><div style={{display:'flex',justifyContent:'space-between',marginTop:6,fontSize:'.68rem',color:'#7a8290'}}><span>कवरेज {row.coveragePercent}%</span><span>{row.hasPerformanceData?`Accuracy ${row.performancePercent}%`:'Quiz pending'}</span></div></div>)}</div></section><aside style={{background:'linear-gradient(145deg,#202632,#333b49)',color:'#fff',borderRadius:16,padding:18}}><div style={{fontSize:'.72rem',fontWeight:800,letterSpacing:'.08em',opacity:.72}}>NEXT BEST ACTION</div><h3 style={{margin:'7px 0 8px',fontSize:'1.2rem'}}>{action}</h3><p style={{margin:0,color:'#cbd0d8',lineHeight:1.55,fontSize:'.82rem'}}>{actionDetail}</p><div style={{marginTop:18,borderTop:'1px solid rgba(255,255,255,.12)',paddingTop:14}}><div style={{fontSize:'.72rem',fontWeight:800,opacity:.72,marginBottom:8}}>FOCUS AREAS</div>{weakest.map(row=><div key={row.id} style={{display:'flex',justifyContent:'space-between',gap:10,padding:'8px 0',borderBottom:'1px solid rgba(255,255,255,.08)'}}><span>{row.icon} {row.name}</span><strong>{row.readiness}%</strong></div>)}</div></aside></div><div className="meter-stats"><span>📚 {metrics.topicsStarted}/{metrics.totalTopics} topics started</span><span>🧩 {metrics.completedStages}/{metrics.totalStages} stages</span><span>📝 {metrics.quizAttempts} quiz attempts</span><span>✅ {metrics.correctAnswers} correct answers</span></div></div></Simple>
}
