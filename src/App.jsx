...
const subjects=[
{id:'math',name:'गणित',icon:'∑',desc:'संख्या, बीजगणित, ज्यामिति और तर्क',chapters:['संख्या पद्धति','बहुपद','निर्देशांक ज्यामिति','दो चरों वाले रैखिक समीकरण','यूक्लिड की ज्यामिति का परिचय']},
...
function ChapterPage({subject,chapter,back,addXp,finishSession}){const[mode,setMode]=useState(null);const baseData=getChapterContent(subject.name,chapter);const isPolynomial=subject.name==='गणित'&&chapter==='बहुपद';const isCoordinate=subject.name==='गणित'&&chapter==='निर्देशांक ज्यामिति';const isLinear=subject.name==='गणित'&&chapter==='दो चरों वाले रैखिक समीकरण';const isEuclid=subject.name==='गणित'&&chapter==='यूक्लिड की ज्यामिति का परिचय';const data=isPolynomial?{goal:chapter2Learning.goal,lessons:chapter2Learning.sections}:isCoordinate?{goal:chapter3Learning.goal,lessons:chapter3Learning.lessons}:isLinear?chapter4Learning:isEuclid?chapter5Learning:baseData;if(mode==='learn')return <LearningEngine subject={subject} chapter={chapter} data={data} onBack={()=>setMode(null)} addXp={addXp} finishSession={finishSession}/>;if(['practice','challenge','test'].includes(mode)){if(isPolynomial)return <PolynomialEngine subject={subject} chapter={chapter} mode={mode} onBack={()=>setMode(null)} addXp={addXp}/>;if(isCoordinate)return <CoordinateEngine chapter={chapter} mode={mode} onBack={()=>setMode(null)} addXp={addXp}/>;if(isLinear)return <LinearEquationEngine subject={subject} chapter={chapter} mode={mode} onBack={()=>setMode(null)} addXp={addXp}/>;if(isEuclid)return <EuclidEngine chapter={chapter} mode={mode} onBack={()=>setMode(null)} addXp={addXp}/>;return <StudyEngine subject={subject} chapter={chapter} mode={mode} onBack={()=>setMode(null)} addXp={addXp}/>;}const modes=[['learn','📖','सीखें','सरल explanation + examples',20],['practice','📝','अभ्यास','Concept समझने के बाद प्रश्न',15],['challenge','🔥','चुनौती','थोड़े कठिन प्रश्न',25],['test','🎯','टेस्ट','समयबद्ध self-assessment',15]];return <Simple title={chapter} back={back}><div className="chapter-hero-card"><span>📚</span><div><strong>{subject.name}</strong><small>{data?.goal||'अध्याय अध्ययन'}</small></div></div><div className="mode-grid">{modes.map(([id,ic,t,d,x])=><button className="mode-card pressable" key={id} onClick={()=>setMode(id)}><span className="mode-icon">{ic}</span><strong>{t}</strong><span>{d}</span><em>+{x} XP</em><b>→</b></button>)}</div></Simple>}
