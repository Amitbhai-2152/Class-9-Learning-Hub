...
function localProgressUpdate(kind,value){
  try{
    const current=JSON.parse(localStorage.getItem('class9-progress')||'{}');
    ...
    if(kind==='xp'){safe.xp+=value;safe.dailyXp=Math.min(safe.goal,safe.dailyXp+value)}else safe.sessions=[...safe.sessions,value].slice(-100);
    localStorage.setItem('class9-progress',JSON.stringify(safe));
  }catch{}
}
...
function openOriginalChapter(){document.querySelector('.subject-section .chapter-grid .chapter-card')?.click()}

function actionButton(label,mode,topic){
  const button=document.createElement('button');button.className='hindi-topic-action pressable';button.type='button';button.textContent=label;button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();writeSelection(topic.title,mode);openOriginalChapter()});return button;
}

function createTopicCard(topic,index,bookTitle){
  const card=document.createElement('article');card.className='chapter-card hindi-enhanced-card';card.dataset.topic=topic.title;
  ...
  const tags=card.querySelector('.hindi-card-tags');(topic.focus||topic.skills||[]).slice(0,3).forEach(item=>{const span=document.createElement('span');span.textContent=item;tags.appendChild(span)});
  const actions=card.querySelector('.hindi-topic-actions');[['📖 सीखें','learn'],['📝 अभ्यास','practice'],['🔥 चुनौती','challenge'],['🎯 टेस्ट','test']].forEach(([label,mode])=>actions.appendChild(actionButton(label,mode,topic)));
...
function enhanceSubject(){
  ...
  const grid=document.querySelector('.subject-section .chapter-grid');if(!grid)return false;
  ...
  subjectHost=document.createElement('div');subjectHost.className='hindi-enhanced-section';
  const intro=document.createElement('div');intro.className='hindi-section-intro';intro.innerHTML='<small>कक्षा 9 • बिहार बोर्ड हिन्दी</small><h2>गोधूली, वर्णिका और व्याकरण</h2><p>पाठ समझें, प्रश्न हल करें और टेस्ट के बाद हर उत्तर की समीक्षा करें। किसी भी पाठ के सामने mode चुनें।</p>';subjectHost.appendChild(intro);
  topicGroups().forEach(([,title,items])=>{const heading=document.createElement('div');heading.className='hindi-group-heading';heading.innerHTML=`<span>${title}</span><b>${items.length} पाठ</b>`;subjectHost.appendChild(heading);const cards=document.createElement('div');cards.className='hindi-enhanced-grid';items.forEach((topic,i)=>cards.appendChild(createTopicCard(topic,i,title)));subjectHost.appendChild(cards)});
  grid.parentElement.insertBefore(subjectHost,grid.nextSibling);return true;
}

function cleanupEngine(){if(engineRoot){try{engineRoot.unmount()}catch{}engineRoot=null}if(engineMount?.parentNode)engineMount.parentNode.removeChild(engineMount);engineMount=null;clearSelection()}

function enhanceChapter(){
  if(!isChapterPage())return false;
  const selection=readSelection();if(!selection?.chapter)return false;
  const content=document.querySelector('.page-content');if(!content)return false;
...
function tick(){
  ...
  if(page!==lastPage){
    if(page&&page!=='हिन्दी'&&!readSelection()?.chapter)cleanupEngine();
